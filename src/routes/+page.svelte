<script>
    ...
    let game = [undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined];
    /** @type {Array<number | undefined>} */
    let cardOwners = [undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined, undefined];

    /** @type {boolean} */
    let isPlayer2Turn = Math.random() < 0.5;

    /** @type {Card | undefined} */
    let selectedCard;

    /** @type {[Array<Card | undefined>, Array<Card | undefined>]}*/
    let decks = [Array.from(new Array(5)).map(randomCard), Array.from(new Array(5)).map(randomCard)];
    ...
    <div class="absolute left-0 top-0">
        <Stack>
            {#each decks[0] as card}
                {#if card}
                    <GameCard card={card} selectable={!isPlayer2Turn} onclick={() => selectP1(card)} selected={card === selectedCard} color='blue' />
                {/if}
            {/each}
        </Stack>
    </div>
    ...
    <div class="absolute right-0 top-0">
        <Stack>
            {#each decks[1] as card}
                {#if card}
                    <GameCard card={card} selectable={isPlayer2Turn} onclick={() => selectP2(card)} selected={card === selectedCard} color='red' />
                {/if}
            {/each}
        </Stack>
    </div>
